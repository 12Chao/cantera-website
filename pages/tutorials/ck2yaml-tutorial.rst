...
* The thermodynamic data is read in a fixed format. This means that each
  ...
  If the elemental number of an atom in a thermodynamic entry has more than 3 digits, 
  it will cause a conversion error. To avoid the unsuccessful conversion, the 
  thermodynamic entry of a species with large elemental number should be written as follows::
